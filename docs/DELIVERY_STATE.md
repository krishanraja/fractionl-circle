...
Last verified: 2026-08-11 (release evidence below). Documentation reconciled against source: 2026-09-07.
...
- Front-door fit repair: pull request `#154` merged to `main` as `5ac840e5db13cd927e383d21a9161c1067871760` on 2026-08-19. It fixed five layout faults on the public front door that the fit rule above forbids: a fixed-height composer that clipped its own two-line prompt, a headline that ran over the paragraph beside it at desktop widths, join buttons hanging past the column edge, a misaligned helper line, and a squeezed field between 900px and 1060px. The commit body records them reproduced from 320px through 1920px and re-verified after the fix. Its checks recorded 80 unit tests and 27 pre-existing TypeScript errors, so the `77 passed` and `tsc --noEmit` lines under Verification evidence describe the `#146`/`#147` head, not the current one. No production deployment ID or Release-procedure readback is recorded for `#154`. A read-only readback on 2026-09-07 found the deployed CSS bundle at the production URL carrying the `#154` front-door rules and the `#151` brand asset references, and both brand images served, so both releases are live. Treat both as released-but-not-fully-verified until someone resolves the active Vercel deployment and repeats the full readback. Nothing has merged to `main` since `#154` as of 2026-09-07.
