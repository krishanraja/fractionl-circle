...
- Typography decision: product, marketing, legal, settings, recovery UI, and social metadata use one native system family. The retired Archivo and Satoshi loading paths and unused font files were removed.
- Share-image decision: `public/og-image.png` is generated by `npm run generate:og` from the same Circle plane, mark, system font, and plain-English promise. It is deterministic brand output, not a second design artifact.
