...
**Last reviewed:** 2026-06-07. **Status: DRAFT — for counsel review.** Derived from the live data model. The authoritative machine list of user-owned tables is `_dsar_user_tables()` in migration `20260602000002_compliance_hardening.sql`.
...
| Idea capture & matching | voice transcripts, idea text, identity inference (motivation type, journey stage, identity statement) | Users | Contract | Life of account | `ideas`, `activity_logs`, `matches`, `moves`, `user_profiles` (identity columns) |
